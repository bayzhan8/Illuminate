...
A **linear program** is a planning problem in which everything is
proportional: a workshop with limited stock, several things it can build, a
known profit for each, and the question of how much of each to build. Since the
1940s the standard way to solve one has been the **simplex method**, which walks
from corner to corner of the region of legal plans, improving as it goes, and
stops when no neighbouring corner is better. [The previous
guide](../corners-vs-centre/) builds it; all you need here is that shape — a
walk, one step at a time.

This guide is about why that shape became a problem, and what replaced it.

Start with the symptom. Two methods, the same small workshop, the same starting
point, the same step sizes. Their inner loops cost the same to run and differ by
one term in one line.
...
on the last. A machine that can do ten thousand things at once is no help when
you only have one thing to do until it finishes.
