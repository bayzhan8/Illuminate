...
LINE_STYLE = {
    "planks": dict(color=INK, linewidth=1.6),
    "hours": dict(color=INK, linewidth=1.6),
    "saw time": dict(color=INK, linewidth=1.6),
}
...
def draw_constraint(ax, lp: LP, i: int, label=True, color=INK, lw=1.4,
...
def callout(ax, text, point, at, color=INK, ha="center"):
    ...
    return ax.annotate(
        ...
        bbox=dict(boxstyle="square,pad=0.5", facecolor=PAPER, edgecolor=color,
                  linewidth=1.1),
...
def draw_axes(ax, xlabel="tables built", ylabel="chairs built"):
    ...
    ax.set_xlim(x0, x1)
    ax.set_ylim(y0, y1)
    ax.set_xlabel(xlabel, fontsize=10, color=INK2, labelpad=7)
    ax.set_ylabel(ylabel, fontsize=10, color=INK2, labelpad=7)
...
def mark_plan(ax, point, label=None, color=PLAN, size=9.5):
    ...
    dot, = ax.plot([x], [y], "o", markersize=size, color=color,
                   markeredgecolor=PAPER, markeredgewidth=2.0, zorder=7)
    if label:
